...
{
    [Header("Item Upgrade")]
    [SerializeField] protected int maxUpgradeLevel = 9;
    ...
    protected virtual bool UpgradeItem(int itemIndex)
    {
        ...
        List<ItemRecipe> upgradeLevels = itemInventory.itemProfileSO.upgrageLevels;
        if (!this.ItemUpgradeable(upgradeLevels)) return false;
        if (!this.HasEnoughIngredients(upgradeLevels, itemInventory.upgradeLevel)) return false;
    ...
    protected virtual bool HasEnoughIngredients(List<ItemRecipe> upgradeLevels, int currentLevel)
    {
        ItemCode itemCode;
        int itemCount;
        ...
        {
            Debug.Log("Item can't upgrade anymore, current level: " + currentLevel);
            return false;
        }
